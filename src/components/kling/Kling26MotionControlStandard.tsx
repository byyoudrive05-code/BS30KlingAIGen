...
interface Kling26MotionControlStandardProps {
  user: User;
...
export default function Kling26MotionControlStandard({ user, currentCredits, onGenerate, reusedPrompt, onPromptUsed, hasProcessingVideo }: Kling26MotionControlStandardProps) {
  ...
  const [pricePerSecond, setPricePerSecond] = useState(0.07);
  ...
  useEffect(() => {
    const loadPricing = async () => {
      const { data } = await supabase
        ...
        .eq('variant', 'motion-control-standard')
    ...
    const checkAccess = async () => {
      const accessResult = await checkModelAccess(user, 'v2.6', 'motion-control-standard');
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-video`,
        {
          ...
          body: JSON.stringify({
            ...
            modelType: 'kling',
            modelVersion: 'v2.6',
            variant: 'motion-control-standard',
            prompt,
            imageUrl,
            videoUrl,
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      <h2 className="text-xl font-bold text-gray-800 mb-6">Kling v2.6 - Motion Control Standard</h2>
