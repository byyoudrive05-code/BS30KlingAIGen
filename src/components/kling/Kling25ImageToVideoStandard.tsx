...
interface Kling25ImageToVideoStandardProps {
  user: User;
...
export default function Kling25ImageToVideoStandard({ user, currentCredits, onGenerate, reusedPrompt, onPromptUsed, hasProcessingVideo, processingCount = 0 }: Kling25ImageToVideoStandardProps) {
  ...
  useEffect(() => {
    const loadPricing = async () => {
      const { data } = await supabase
        ...
        .select('*')
        .eq('model_type', 'kling')
        .eq('model_version', 'v2.5-turbo')
        .eq('variant', 'image-to-video-standard')
    ...
    const checkAccess = async () => {
      const accessResult = await checkModelAccess(user, 'v2.5-turbo', 'image-to-video-standard');
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      <h2 className="text-xl font-bold text-gray-800 mb-6">Kling v2.5 Turbo - Image to Video Standard</h2>
