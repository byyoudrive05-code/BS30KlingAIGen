...
interface Kling25ImageToVideoProProps {
  user: User;
...
export default function Kling25ImageToVideoPro({ user, currentCredits, onGenerate, reusedPrompt, onPromptUsed, hasProcessingVideo }: Kling25ImageToVideoProProps) {
  ...
  useEffect(() => {
    ...
    const checkAccess = async () => {
      const accessResult = await checkModelAccess(user, 'v2.5-turbo', 'image-to-video-pro');
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg p-6">
      <h2 className="text-xl font-bold text-gray-800 mb-6">Kling v2.5 Turbo - Image to Video Pro</h2>
